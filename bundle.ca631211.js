...
module.hot.dispose(reloadCSS);
module.hot.accept(reloadCSS);
},{"_css_loader":"node_modules/parcel-bundler/src/builtins/css-loader.js"}],"src/script/nameGenerator.ts":[function(require,module,exports) {
"use strict";
...
const firstName = ["Red", "Green", "Blue", "Yellow", "Magenta", "Black", "White", "Orange"];
const lastName = ["Fox", "Apple", "Bunny", "Cherry", "Tree", "Mouse", "Bear", "Star"];

const generateName = () => "".concat(firstName[Math.random() * firstName.length | 0], " ").concat(lastName[Math.random() * lastName.length | 0]);
...
},{}],"src/script/types.ts":[function(require,module,exports) {
"use strict";
...
(function (actions) {
  ...
  actions["incoming"] = "INCONIMG";
  actions["outbound"] = "OUTBOUND";
})(actions = exports.actions || (exports.actions = {}));
},{}],"src/script/logic.ts":[function(require,module,exports) {
"use strict";
...
exports.sendMessageThunk = exports.store = exports.actionCreator = void 0;
...
const reducer = (state, action) => {
  const waitClockImg = document.getElementById("#sand-clock");

  if (state) {
    switch (action.type) {
      case types_1.actions.sending:
        {
      ...
      case types_1.actions.incoming:
        {
          if (action.payload["date"]) {
            state.messages.length > CHAT_LENGTH && state.messages.shift();
            state.messages.push(Object.assign({}, action.payload));
          } else {
            for (const key in action.payload) {
              state.messages.length > CHAT_LENGTH && state.messages.shift();
              state.messages.push(Object.assign({}, action.payload[key]));
            }
          }
...
exports.store.subscribe(() => {
  ...
  const imgs = document.getElementById("#smiles-area").querySelectorAll("img");
  exports.store.getState().messages.forEach((msg, index) => {
    if (!msg.message) {
      return;
    }
    ...
    Array.from(imgs).forEach((img, index) => {
      div.innerHTML = "&#".concat(img.alt);
      msg.message = msg.message.split(div.innerHTML).join("\0::".concat(index, "::\0"));
    });
    div.innerHTML = "";
    div.classList.add("message-window__message-container");
    div.classList.add("message-container");
    const d = new Date(msg.date);
    const dateStr = "".concat(d.toLocaleDateString(), "  ").concat(d.getHours(), ":").concat(d.getMinutes(), ":").concat(d.getSeconds());
    const p = document.createElement("p");
    p.classList.add("message-container__message");

    if (name === "".concat(msg.nickname || msg.name)) {
      p.classList.add("message-container__message--our");
      p.innerHTML = "".concat(dateStr, "  <  ");
    } else {
      p.innerHTML = "".concat(dateStr, "  >  ").concat(msg.nickname || msg.name, ": ");
    }

    div.appendChild(p);
    msg.message.split("\u0000").forEach((el, index) => {
      if (el.match(/^::[0-9]+::$/)) {
...
const sendMessageThunk = (msg, sendMessage) => {
  return dispatch => __awaiter(void 0, void 0, void 0, function* () {
    dispatch((0, exports.actionCreator)(types_1.actions.sending, msg));
    yield sendMessage(msg);
    dispatch((0, exports.actionCreator)(types_1.actions.outbound, msg));
  });
};

exports.sendMessageThunk = sendMessageThunk;
},{"redux":"node_modules/redux/es/redux.js","redux-thunk":"node_modules/redux-thunk/es/index.js","./types":"src/script/types.ts"}],"src/script/messagesAPI.js":[function(require,module,exports) {
"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.observeWithEventSource = observeWithEventSource;
...
async function getMessagesList() {
  return fetch("".concat(config.firebaseBaseUrl, "/").concat(config.firebaseCollection), {
    headers: {
      Accept: "application/json",
      "Content-Type": "application/json"
    }
  }).then(response => response.json()).then(data => Object.values(data).map(el => _objectSpread(_objectSpread({}, el), {}, {
...
function observeWithXHR(cb) {
  ...
  xhr.open("POST", "".concat(config.firebaseBaseUrl, "/").concat(config.firebaseCollection), true);
...
function observeWithEventSource(cb) {
  ...
  const evtSource = new EventSource("".concat(config.firebaseBaseUrl, "/").concat(config.firebaseCollection));
...
},{}],"src/script/events.ts":[function(require,module,exports) {
"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.startObserver = exports.imgEvent = exports.keyPressInputEvent = exports.sendEvent = void 0;
...
const sendEvent = () => {
  const msg = document.getElementById("#send-input").innerHTML;
  ...
  const d = new Date();
  const sendingMsg = {
    date: "".concat(d.toLocaleDateString(), ":").concat(d.getHours(), ".").concat(d.getMinutes(), ".").concat(d.getSeconds()),
    message: msg,
    name: name
  };
  logic_1.store.dispatch((0, logic_1.sendMessageThunk)(sendingMsg, messagesAPI_1.sendMessage));
  document.getElementById("#send-input").innerHTML = "";
};

exports.sendEvent = sendEvent;

const keyPressInputEvent = e => {
  const sendInput = document.getElementById("#send-input");
  sendInput.innerHTML === "<br>" && (sendInput.innerHTML = "");
  e.keyCode === 13 && (0, exports.sendEvent)();
};

exports.keyPressInputEvent = keyPressInputEvent;

const imgEvent = e => {
  const img = e.target;
  const sendInput = document.getElementById("#send-input");
  sendInput.innerHTML = sendInput.innerHTML.split("<br>").join("");
  sendInput.innerHTML += "&#".concat(img.alt);
};

exports.imgEvent = imgEvent;

const startObserver = () => {
  (0, messagesAPI_1.observeWithEventSource)(msg => {
    logic_1.store.dispatch((0, logic_1.actionCreator)(types_1.actions.incoming, msg));
  });
};

exports.startObserver = startObserver;
},{"./logic":"src/script/logic.ts","./types":"src/script/types.ts","./messagesAPI":"src/script/messagesAPI.js"}],"src/script/chat.ts":[function(require,module,exports) {
"use strict";
...
const startChat = () => {
  document.getElementById("#button-send").addEventListener("click", events_1.sendEvent);
  document.getElementById("#send-input").addEventListener("keypress", events_1.keyPressInputEvent);
  Array.from(document.getElementById("#smiles-area").querySelectorAll("img")).forEach(img => {
    img.addEventListener("click", events_1.imgEvent);
  });
  document.getElementById("#name-input").value = (0, nameGenerator_1.generateName)();
  (0, events_1.startObserver)();
};

exports.startChat = startChat;
},{"./nameGenerator":"src/script/nameGenerator.ts","./events":"src/script/events.ts"}],"src/script/bundle.ts":[function(require,module,exports) {
"use strict";
...
(0, chat_1.startChat)();
},{"../scss/reset.scss":"src/scss/reset.scss","../scss/style.scss":"src/scss/style.scss","./chat":"src/script/chat.ts"}],"node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
